...
class ActualizarDeuda:
    ...
    async def ejecutar(self, id_deuda: int, info_nueva: ActualizarDeudaSchema) -> Deuda:
        info_vieja = await self.repo_obtener.obtener_por_id(id_deuda)
        ...
        deuda = await self.repo_actualizar.actualizar(info_vieja)
