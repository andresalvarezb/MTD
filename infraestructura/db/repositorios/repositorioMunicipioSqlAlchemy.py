...
from core.interfaces.repositorioMunicipio import RepositorioDepartamento, RepositorioMunicipio


class RepositorioDepartamentoSqlAlchemy(RepositorioDepartamento):
    ...
    def obtener(self, departamento: Departamento):
        existe = self.db.query(DepartamentoORM).filter_by(nombre=departamento.nombre).first()
        if existe:
            return existe
        else:
            return None
...
class RepositorioMunicipioSqlAlchemy(RepositorioMunicipio):
    ...
    def obtener(self, municipio: Municipio):
        existe = self.db.query(MunicipioORM).filter_by(nombre=municipio.nombre).first()
        if existe:
            return existe
        else:
            return None
