...
from app.api.esquemas.historialLaboralUsuario import HistorialLaboralResponseSchema
from app.api.esquemas.cuentaBancaria import CuentaBancariaResponseSchema
...
class ObtenerCuentaPorPagar:
    ...
    def ejecutar(self, id_cuenta_por_pagar: int):
        ...
        if not cuenta:
            raise Exception("cuenta no encontrada para actualizar")

        if not cuenta.id:
            raise Exception("id de cuenta no encontrado para actualizar")
        ...
        return CuentaPorPagarResponseSchema(
            ...
            total_descuentos=Decimal(cuenta.total_descuentos) if cuenta.total_descuentos else Decimal(0),
            total_a_pagar=Decimal(cuenta.total_a_pagar) if cuenta.total_a_pagar else Decimal(0),
